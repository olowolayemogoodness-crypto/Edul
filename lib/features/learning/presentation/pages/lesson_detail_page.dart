...
class _LessonDetailPageState extends State<LessonDetailPage> {
  ...
  Map<String, dynamic> _getLessonData(String lessonId) {
    if (lessonId.startsWith('mts102_')) return getMTS102LessonData(lessonId);
    if (lessonId.startsWith('csc102_')) return getCSC102LessonData(lessonId);
    if (lessonId.startsWith('bio102_')) return getBIO102LessonData(lessonId);
    if (lessonId.startsWith('che102_')) return getCHE102LessonData(lessonId);
    if (lessonId.startsWith('phy102_')) return getPHY102LessonData(lessonId);
    ...
    if (lessonId.startsWith('mts104_')) return getMTS104LessonData(lessonId);
  ...
  Widget build(BuildContext context) {
    if (questions.isEmpty) {
      return Scaffold(
        ...
        body: Center(child: Text('Loading lesson...', style: GoogleFonts.dmSans())),
      );
    }
    if (!showQuiz) return _buildTeachingPage(context);
    if (quizComplete) return _buildResultsPage(context);
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(widget.lessonName, style: GoogleFonts.dmSans(fontSize: 16, fontWeight: FontWeight.w600)),
    ...
    for (final line in lines) {
      if (line.startsWith('# ')) {
        widgets.add(Padding(
          ...
          child: Text(line.replaceFirst('# ', ''), style: GoogleFonts.dmSans(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
        ));
      } else if (line.startsWith('## ')) {
        widgets.add(Padding(
          padding: const EdgeInsets.only(top: 16, bottom: 8),
          child: Text(line.replaceFirst('## ', ''), style: GoogleFonts.dmSans(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
        ));
      } else if (line.startsWith('• ')) {
        widgets.add(Padding(
          padding: const EdgeInsets.only(bottom: 8, left: 12),
          child: Text(line.replaceFirst('• ', ''), style: GoogleFonts.dmSans(fontSize: 13, color: AppColors.textSecondary, height: 1.5)),
        ));
      } else if (line.startsWith('💡 ')) {
        ...
        widgets.add(Padding(
          ...
          child: Text(line, style: GoogleFonts.dmSans(fontSize: 13, color: AppColors.textSecondary, height: 1.5)),
        ));
      }
    }
    return widgets;
          ...
          child: Column(children: [
            Container(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  GestureDetector(onTap: () => Navigator.pop(context), child: const Icon(Icons.arrow_back_rounded, size: 20)),
                  Text(widget.lessonName, style: GoogleFonts.dmSans(fontSize: 16, fontWeight: FontWeight.w600)),
                  const SizedBox(width: 20),
                ],
              ),
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                Text('${currentQuestionIndex + 1}/${questions.length}', style: GoogleFonts.dmSans(fontSize: 12, color: AppColors.textTertiary)),
              ]),
            ),
            Padding(
              ...
              child: Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(currentQuestion['question'], style: GoogleFonts.dmSans(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                  ...
                  ...List.generate(currentQuestion['options'].length, (index) {
                    ...
                    return Padding(
                      ...
                      child: GestureDetector(
                        ...
                        child: Container(
                          padding: const EdgeInsets.all(12),
                          decoration: BoxDecoration(color: bgColor, border: Border.all(color: borderColor, width: 1.5), borderRadius: BorderRadius.circular(12)),
                          child: Row(children: [
                            Container(
                              width: 24, height: 24,
                              decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: borderColor, width: 2), color: isSelected ? borderColor : Colors.transparent),
                  ...
                  if (showResult) Padding(
                    ...
                    child: Text(isCorrect ? '✅ Correct!' : '❌ Not quite right',
                      style: GoogleFonts.dmSans(fontSize: 14, fontWeight: FontWeight.w600, color: isCorrect ? AppColors.success : const Color(0xFFEF4444))),
                  ),
                  if (showResult) Padding(
                    padding: const EdgeInsets.only(top: 16),
                    child: SizedBox(
          ...
          child: Column(children: [
            ...
            Padding(
              ...
              child: Column(children: [
                ...
                Text(passed ? 'You passed the quiz!' : 'You need 80% to pass. Try again!',
                ...
                Container(
                  ...
                  child: Column(children: [
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      Text('Score', style: GoogleFonts.dmSans(fontSize: 14, color: AppColors.textTertiary)),
                      Text('$percentage%', style: GoogleFonts.dmSans(fontSize: 20, fontWeight: FontWeight.w700,
                        color: passed ? AppColors.success : const Color(0xFFEF4444))),
                    ]),
                    const SizedBox(height: 12),
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      Text('Correct answers', style: GoogleFonts.dmSans(fontSize: 14, color: AppColors.textTertiary)),
                      Text('$correctAnswers/${questions.length}', style: GoogleFonts.dmSans(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                    ]),
                  ]),
                ),
                ...
                if (!passed)
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Text('Try Again', style: GoogleFonts.dmSans(fontSize: 14, fontWeight: FontWeight.w700, color: Colors.white)),
                    ),
                  )
                else
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        await StudyTimeService.recordQuiz(
                      ...
                      style: ElevatedButton.styleFrom(backgroundColor: AppColors.accent, padding: const EdgeInsets.symmetric(vertical: 14)),
                      child: Text('Next Lesson 🚀', style: GoogleFonts.dmSans(fontSize: 14, fontWeight: FontWeight.w700, color: Colors.white)),
                    ),
                  ),
              ]),
            ),
